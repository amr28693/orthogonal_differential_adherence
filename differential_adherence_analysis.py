...
def load_data():
    ...
    # Sensitivity analysis with complete-case (skipna=False) reported in manuscript.
...
def print_descriptives(df, log):
    ...
    lines.append("-" * 72)
    hdr = f"{'Group':<28} {'N':>5} {'d Mean':>8} {'d SD':>7} {'%Int':>6} {'Sum Mean':>9} {'Sum SD':>7}"
    lines.append(hdr)
    lines.append("-" * 72)
...
def run_statistics(df, log):
    ...
    lines.append(f"  Shared variance: {r**2*100:.1f}%")
